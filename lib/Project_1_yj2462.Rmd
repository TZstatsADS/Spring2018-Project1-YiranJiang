...
packages.used=c("qdap", "lattice", "grid", 
                ...
                "png", "ggplot","MASS", "devtools","reshape")
...
# Step 1: Data cleaning: read the GDP data and clean the data.
...
# Step 2: Sentiment analysis: analyze the sentiment in each inauguration speech.
...
```
This Heatmap shows a breif pattern of how the sentiment in the inauguration speech over years are clustered together.


# Step 3: Neural Network training: Train a Neural Network using BP to fit the GDP data.
...
The I1~I10 represents 10 inputs nodes, the H1~H10 represents 10 nodes in the hidden layer and B1~B2 are the bias nodes. O1 is the output node, which is the GDP.
...
------------------------
From the result, we could see that our model predicts quite well the GDP Growth in each year, despite some "overestimate" in some years. 

One intresting result is that our model successfully predicted the pit of GDP Growth in the year 2009, the year after the global financial crises happened. This suggests that in the inauguration speech, the president could somewhat mentioned such trend by conducting certain sentiments in the speech.
